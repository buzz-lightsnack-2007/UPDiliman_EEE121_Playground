...
int main() {
	vector<vector<int>> lists = {
		...
		{0, 86, 1, 5, 2, 59, 3, 95, 4, 6, 84, 8, 26, 78, 27, 25, 89, 8, 5, 32, 6, 1, 7, 72, 8, 3, 82, 22, 16, 66, 21, 18, 43, 39, 9, 85, 10, 70, 11, 14, 12, 72, 13, 18, 87, 27, 2, 19, 32, 72, 43, 20, 14, 21, 15, 68, 16, 96, 17, 62, 99, 32, 25, 45, 87, 60, 93, 34, 18, 42, 19, 60, 20, 50, 21, 33, 22, 88, 20, 48, 71, 8, 33, 95, 50, 31, 23, 27, 24, 6, 25, 78, 26, 1, 55, 79, 41, 71, 55, 82, 76, 83, 27, 3, 28, 66, 29, 27, 30, 95, 31, 86, 92, 97, 38, 10, 24, 57, 69, 73, 32, 34, 33, 15, 34, 45, 35, 46, 83, 72, 50, 15, 3, 23, 49, 26, 36, 45, 37, 8, 38, 23, 39, 40, 40}
	};
	vector<int> dimensions = {4, 7, 4};
	vector<int> maximum = {40, 112, 40};
	vector<int> sizes;
	vector< vector<vector<int>>> els;
	vector< vector<vector<int>>> adjmats;
	vector< vector<vector<int>>> sdts; // Shortest distance tables

	for (int size = 0; sizes.size() < dimensions.size(); size = 0) {
		size = dimensions[sizes.size()];
		sizes.push_back(size * 2 + 1);
	};
	...
	for (int list_ID = 0; list_ID < lists.size(); list_ID++) {
		...
		sdts.push_back(djikstra(&(adjmats[list_ID]), 20, 9999999));
